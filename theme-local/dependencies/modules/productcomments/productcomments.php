...
class ProductComments extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->version = '10.0.0';
        $this->author = 'modified by MyPresta';
        $this->mypresta_link = '';
    ...
    public function install($keep = true)
    {
        ...
        if (parent::install() == false || !$this->registerHook('displayCommentsExtra')  || !$this->registerHook('displayProductTab') || !$this->registerHook('displayProductTabContent') || !$this->registerHook('displayFooterProduct') || !$this->registerHook('displayProductAdditionalInfo') || !$this->registerHook('header') || !$this->registerHook('displayRightColumnProduct') || !$this->registerHook('displayProductListReviews') || !Configuration::updateValue('PRODUCT_COMMENTS_MINIMAL_TIME', 30) || !Configuration::updateValue('PRODUCT_COMMENTS_COMMENTS_PER_PAGE', 5) || !Configuration::updateValue('PRODUCT_COMMENTS_ALLOW_GUESTS', 0) || !Configuration::updateValue('PRODUCT_COMMENTS_MODERATE', 1)) {
            return false;
        }
    ...
    {
        if (!parent::uninstall() || ($keep && !$this->deleteTables()) || !Configuration::deleteByName('PRODUCT_COMMENTS_MODERATE') || !Configuration::deleteByName('PRODUCT_COMMENTS_ALLOW_GUESTS') || !Configuration::deleteByName('PRODUCT_COMMENTS_MINIMAL_TIME') || !Configuration::deleteByName('PRODUCT_COMMENTS_COMMENTS_PER_PAGE') || !$this->unregisterHook('displayRightColumnProduct') || !$this->unregisterHook('header') || !$this->unregisterHook('displayCommentsExtra') || !$this->unregisterHook('displayProductTab') || !$this->unregisterHook('displayProductTabContent') || !$this->unregisterHook('displayFooterProduct') || !$this->unregisterHook('displayProductAdditionalInfo') || !$this->unregisterHook('top') || !$this->unregisterHook('displayProductListReviews')) {
            return false;
        }
...
//* v8 
	private function _setBaseUrl()
	{
		...
		foreach ($keysCriterion as $key) {
			Tools::getValue($key);
		}
		$request = Tools::getAllValues();
		foreach ($request as $key => $value) {
			if (!in_array($key, $keysCriterion)) {
				$this->_baseUrl .= $key . '=' . Tools::getValue($key) . '&';
			}
		}
    ...
    public function hookDisplayCommentsExtra($params)
    {
        ...
        // $cover_image = $this->context->link->getImageLink($product->link_rewrite, $image['id_image'], 'home_default');
        $comment_name = $comment_name = $product->name;
		$comment_desc = $product->description_short;
        ...
        $this->context->smarty->assign(array(
            ...
            'productcomment_name' => $comment_name,
            'productcomment_desc' => $comment_desc,
            //* v8 'homeSize' => Image::getSize(ImageType::getFormatedName('home')),
    ...
    public function hookHeader()
    {
        ...
        $this->context->controller->addCSS($this->_path . 'views/css/productcomments-at.css', 'all');
